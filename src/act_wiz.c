...
void do_wizhelp( CHAR_DATA * ch, char *argument )
{
   ...
   for( level = LEVEL_IMMORTAL - 1; level <= LEVEL_SUPREME + 1; level++ )
   {
      ...
      for( hash = 0; hash < 126; hash++ )
         for( cmd = command_hash[hash]; cmd; cmd = cmd->next )
            if( cmd->level >= LEVEL_HERO && cmd->level <= get_trust( ch ) && cmd->level == level )
            {
               pager_printf( ch, "%-12s", cmd->name );
               if( ++col % 6 == 0 )
                  send_to_pager( "\r\n", ch );
...
void do_bamfin( CHAR_DATA * ch, char *argument )
{
   ...
   if( !argument || argument[0] == '\0' )
   {
      ...
      send_to_char( "&YDefault bamfin set.\r\n", ch );
      return;
   }
...
void do_bamfout( CHAR_DATA * ch, char *argument )
{
   ...
   if( !argument || argument[0] == '\0' )
   {
      ...
      send_to_char( "&YDefault bamfout set.\r\n", ch );
      return;
   }
...
void do_echo( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg, "Thoric" )
       || !str_cmp( arg, "Circe" ) || !str_cmp( arg, "Haus" ) || !str_cmp( arg, "Scryn" ) || !str_cmp( arg, "Blodkai" ) )
   {
...
void do_ostat( CHAR_DATA * ch, char *argument )
{
   ...
   {
      ch_printf_color( ch, "&cFurnitureFlags: &w%s\r\n", flag_string( obj->pIndexData->value[2], fur_pos ) );
   }
   ...
   {
      ch_printf_color( ch, "&cFurnitureFlags: &w%s\r\n", flag_string( obj->value[2], fur_pos ) );
   }
...
/*    if ( !IS_NPC(victim) )
   ...
   if( get_trust( ch ) >= LEVEL_GOD && !IS_NPC( victim ) && victim->desc )
      pager_printf_color( ch, "&cUser: &w%s@%s   Descriptor: %d  &cTrust: &w%d  &cAuthBy: &w%s\r\n",
                          ...
                          victim->trust, victim->pcdata->authed_by != NULL ? victim->pcdata->authed_by : "(unknown)" );
   ...
   pager_printf_color( ch, "&cLevel   : &P%-3d             ", victim->level );
   if( victim->class < MAX_NPC_CLASS && victim->class >= 0 && victim->race < MAX_NPC_RACE && victim->race >= 0 )
      pager_printf_color( ch, "&cClass  : &w%-2.2d/%-10s   &cRace      : &w%-2.2d/%-10s\r\n",
                          victim->class, npc_class[victim->class], victim->race, npc_race[victim->race], victim->position );
   else
      send_to_pager( "\r\n", ch );
...
void do_strip( CHAR_DATA * ch, char *argument )
{
   ...
   if( !count )
      send_to_pager_color( "&GNothing found to take.\r\n", ch );
   return;
}
...
void do_users( CHAR_DATA * ch, char *argument )
{
   ...
   sprintf( buf, "\r\n&CDesc&w|&cCon&w|&GIdle&w| &gPort &w| &YPlayer      &O@HostIP           " );
      ...
      if( arg[0] == '\0' )
      {
         if( get_trust( ch ) >= LEVEL_SUPREME || ( d->character && can_see( ch, d->character ) ) )
         {
            ...
            sprintf( buf,
                     ...
                     d->connected,
                     d->idle / 4,
                     d->port, d->original ? d->original->name : d->character ? d->character->name : "&B(none)&w", d->host );
            if( get_trust( ch ) >= LEVEL_GOD )
      ...
      else
      {
             ...
             && ( !str_prefix( arg, d->host ) || ( d->character && !str_prefix( arg, d->character->name ) ) ) )
         {
            ...
            pager_printf( ch,
                          ...
                          d->idle / 4,
                          d->port,
                          d->original ? d->original->name : d->character ? d->character->name : "&B(none)", d->host );
            buf[0] = '\0';
            if( get_trust( ch ) >= LEVEL_GOD )
...
void do_newbieset( CHAR_DATA * ch, char *argument )
{
   ...
   if( ( victim->class == 0 ) || ( victim->class == 2 ) || ( victim->class == 4 ) || ( victim->class == 7 ) )
   {
...
void do_cset( CHAR_DATA * ch, char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      pager_printf_color( ch, "&WMUD Name: %s\r\n", sysdata.mud_name );
      pager_printf_color( ch, "&WMail:\r\n" );
      pager_printf_color( ch, "  &wRead mail for free          : &W%-7d", sysdata.read_mail_free );
      pager_printf_color( ch, "&wWrite mail for free         : &W%d\r\n", sysdata.write_mail_free );
      pager_printf_color( ch, "  &wRead all mail               : &W%-7d", sysdata.read_all_mail );
      pager_printf_color( ch, "&wTake all mail               : &W%d\r\n", sysdata.take_others_mail );
/*
	pager_printf_color(ch, "&WPfile:\r\n" );
	pager_printf_color(ch, "  &wPfile autocleanup           : &W%s\r\n",
			   sysdata.CLEANPFILES ? "ON" : "off" );
	pager_printf_color(ch, "  &wNewbie purge days           : &W%-7d",
			   sysdata.newbie_purge );
	pager_printf_color(ch, "&wRegular purge days          : &W%d\r\n",
			   sysdata.regular_purge );
*/
      pager_printf_color( ch, "&WChannels:\r\n" );
      pager_printf_color( ch, "  &wStaff                       : &W%-7d", sysdata.think_level );
      pager_printf_color( ch, "&wThink                       : &W%d\r\n", sysdata.think_level );
      pager_printf_color( ch, "  &wLog                         : &W%-7d", sysdata.log_level );
      pager_printf_color( ch, "&wBuild                       : &W%d\r\n", sysdata.build_level );
      pager_printf_color( ch, "&WBuilding:\r\n" );
      pager_printf_color( ch, "  &wPrototype modification      : &W%-7d", sysdata.level_modify_proto );
      pager_printf_color( ch, "&wPlayer msetting             : &W%d\r\n", sysdata.level_mset_player );
      pager_printf_color( ch, "&WClubs:\r\n" );
      pager_printf_color( ch, "  &wOverseer                    : &W%s\r\n", sysdata.guild_overseer );
      pager_printf_color( ch, "  &wAdvisor                     : &W%s\r\n", sysdata.guild_advisor );
      pager_printf_color( ch, "&WBan Data:\r\n" );
      pager_printf_color( ch, "  &wBan Site Level              : &W%-7d", sysdata.ban_site_level );
      pager_printf_color( ch, "&wBan Class Level             : &W%d\r\n", sysdata.ban_class_level );
      pager_printf_color( ch, "  &wBan Race Level              : &W%d\r\n", sysdata.ban_race_level );
      pager_printf_color( ch, "&WDefenses:\r\n" );
      pager_printf_color( ch, "  &wDodge_mod                   : &W%-7d", sysdata.dodge_mod );
      pager_printf_color( ch, "&wParry_mod                   : &W%d\r\n", sysdata.parry_mod );
      pager_printf_color( ch, "  &wTumble_mod                  : &W%d\r\n", sysdata.tumble_mod );
      pager_printf_color( ch, "&WOther:\r\n" );
      pager_printf_color( ch, "  &wForce on players            : &W%-7d", sysdata.level_forcepc );
      pager_printf_color( ch, "&wPrivate && immproof override : &W%d\r\n", sysdata.level_override_private );
      ...
      pager_printf_color( ch, "  &wBestowed command level diff.: &W%-7d", sysdata.bestow_dif );
      ...
      pager_printf_color( ch, "  &wIdents retries              : &W%-7d", sysdata.ident_retries );
/*
	pager_printf_color(ch, "&wDouble Exp is               : &W%s\r\n",
			   sysdata.double_exp ? "ON" : "off" );
*/
      pager_printf_color( ch, "&wAuthorization is            : &W%s\r\n", sysdata.WAIT_FOR_AUTH ? "ON" : "off" );
/*
	pager_printf_color(ch, "&wNever Empty is              : &W%s\r\n",
			   sysdata.neverempty ? "ON" : "off" );
*/
      pager_printf_color( ch, "  &wAllow Immortals in arena    : &W%-7s",
                          ( sysdata.imm_arena == TRUE ) ? "&WYES    &w" : "&Wno     &w " );
      pager_printf_color( ch, "  &wWebServer                   : &W%-7s",
                          ( sysdata.webtoggle == TRUE ) ? "&WON     &w" : "&Woff    &w" );
      pager_printf_color( ch, "&wWebPort                     : &W%d\r\n", sysdata.webport );
/*
	pager_printf_color(ch, "&wWebCounter                  : &W%s\r\n", 
			   (sysdata.webcounter==TRUE) ? "&GON&w" : "&Roff&w" );
	pager_printf_color(ch, "  &wHomePage: &W%s\r\n",
			   sysdata.homepage );
*/
      pager_printf_color( ch, "  &wMultiplayer limit           : &W%-7d", sysdata.mplimit );
      pager_printf_color( ch, "&wPlayerkiller limit          : &W%d\r\n", sysdata.pklimit );
/*
    pager_printf_color(ch, "\r\n&WMud_name: %s", sysdata.mud_name );
    pager_printf_color(ch, "\r\n&WAuthorization: %s", sysdata.WAIT_FOR_AUTH ? "Enabled" : "Disabled");
    ...
    pager_printf_color(ch, "  &wTake all mail: &W%d\r\n", sysdata.take_others_mail );
    pager_printf_color(ch, "&WChannels:\r\n  &wStaff: &W%d   &wThink: &W%d   &wLog: &W%d   &wBuild: &W%d\r\n",
...
*/
   ...
   if( !str_cmp( arg, "mplimit" ) )
   {
      sysdata.mplimit = level;
      send_to_char( "Ok.\r\n", ch );
      return;
   }

   if( !str_cmp( arg, "pklimit" ) )
   {
      sysdata.pklimit = level;
      send_to_char( "Ok.\r\n", ch );
      return;
   }
   ...
   if( !str_cmp( arg, "authorize" ) )
   {
      sysdata.WAIT_FOR_AUTH = !sysdata.WAIT_FOR_AUTH;

      if( sysdata.WAIT_FOR_AUTH )
         send_to_char( "Authorization now enabled.\r\n", ch );
      else
         send_to_char( "Authorization now disabled.\r\n", ch );
...
void do_qpset( CHAR_DATA * ch, char *argument )
{
   ...
   if( nifty_is_name_prefix( arg2, "give" ) )
   {
      ...
      if( ( str_cmp( ch->pcdata->council_name, "Quest Council" )
            || str_cmp( ch->pcdata->council2_name, "Quest Council" ) ) && ( get_trust( ch ) < LEVEL_DEMI ) )
      {
...
void do_project( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg1, "delete" ) )
   {
      ...
      if( str_cmp( ch->name, "Kali" )
          && ( str_cmp( ch->pcdata->council_name, "Code Council" )
               || str_cmp( ch->pcdata->council2_name, "Code Council" ) ) && get_trust( ch ) < LEVEL_ASCENDANT )
      {
