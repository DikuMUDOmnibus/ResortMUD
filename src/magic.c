...
/* 	mana = IS_NPC(ch) ? 0 : UMAX(skill->min_mana,
   ...
   else
   {
      ...
      else
      {
         ...
         chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_third_cast ) + ( ch->level / 10 * 1.5 ) ) / 2 );
         if( number_percent(  ) < chance )
         {
         ...
         chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_fourth_cast ) + ( ch->level / 10 * 2 ) ) / 2.5 );
         if( number_percent(  ) < chance )
         {
         ...
         chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_fifth_cast ) + ( ch->level / 10 * 2.5 ) ) / 3 );
         if( number_percent(  ) < chance )
         {
         ...
         chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_sixth_cast ) + ( ch->level / 10 * 3 ) ) / 3.5 );
         if( number_percent(  ) < chance )
         {
         ...
         chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_seventh_cast ) + ( ch->level / 10 * 3.5 ) ) / 4 );
         if( number_percent(  ) < chance )
         {
         ...
         chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_eighth_cast ) + ( ch->level / 10 * 4 ) ) / 4.5 );
         if( number_percent(  ) < chance )
         {
...
/*    if ( !IS_NPC( victim ) && !IS_NPC( ch ) )
  ...
  -- What's wrong with a little charming between friends? -- Scion */
   ...
   if( IS_AFFECTED( victim, AFF_CHARM )
       ...
       || level < victim->level || circle_follow( victim, ch ) || saves_spell_staff( chance, victim ) )
   {
      failed_casting( skill, ch, victim, NULL );
      return rSPELL_FAILED;
   }
...
ch_ret spell_enchant_weapon( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   if( obj->item_type != ITEM_WEAPON
       || IS_OBJ_STAT( obj, ITEM_MAGIC ) || IS_OBJ_STAT( obj, ITEM_ARTIFACT ) || obj->first_affect )
   {
...
ch_ret spell_gate( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   char_to_room( victim = create_mobile( temp ), ch->in_room );
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   sprintf( buf, "%s teleported %s to room %d.", ch->name, victim->name, victim->in_room->vnum );
   log_string_plus( buf, LOG_NORMAL, ch->level );
   to_channel( buf, CHANNEL_MONITOR, "Monitor", UMAX( LEVEL_IMMORTAL, ch->level ) );
