...
void do_sset( CHAR_DATA * ch, char *argument )
{
    ...
    * Scion's code! 
    */
   if( get_trust( ch ) > LEVEL_GREATER && !str_cmp( arg1, "copy" ) )
   {
      ...
      for( index = 0; index < top_sn; index++ )
      {
         skill_table[index]->skill_level[class2] =
            ( skill_table[index]->skill_level[class1] <= MAX_LEVEL ? skill_table[index]->skill_level[class1] : 101 );
         skill_table[index]->skill_adept[class2] =
            ( skill_table[index]->skill_adept[class1] <= MAX_LEVEL ? skill_table[index]->skill_adept[class1] : 101 );
      }
   ...
   if( !IS_NPC( victim ) && !IS_NPC( ch ) )
   {
          ...
          && ch != victim )
      {
         sprintf( log_buf, "%s illegally stealing from %s at %d",
                  ( IS_NPC( ch ) ? ch->short_descr : ch->name ), victim->name, victim->in_room->vnum );
         log_string( log_buf );
