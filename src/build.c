...
char *const area_flags[] = {
   "nopkill", "freekill", "noteleport", "prototype", "r4", "r5", "r6", "r7", "r8",
...
char *copy_buffer_nohash( CHAR_DATA * ch )
{
   ...
   {
      strncpy( tmp, ch->editor->line[x], 100 );
      len = strlen( tmp );
      if( tmp && tmp[len - 1] == '~' )
         tmp[len - 1] = '\0';
      else
         strncat( tmp, "\n", 100 );
      smash_tilde( tmp );
      strncat( buf, tmp, MAX_STRING_LENGTH );
   }
   return str_dup( buf );
}
...
void do_goto( CHAR_DATA * ch, char *argument )
{
   ...
   for( fch = in_room->first_person; fch; fch = fch_next )
   {
      ...
      {
         sprintf( buf, "&Y%s&w\r\n", ( ch->pcdata && ch->pcdata->bamfout && ch->pcdata->bamfout[0] != '\0' )
   ...
   for( fch = ch->in_room->first_person; fch; fch = fch_next )
   {
      ...
      if( fch != ch && ( ( get_trust( fch ) >= ( IS_NPC( ch ) ? ch->level : ch->pcdata->wizinvis ) )
                         || !xIS_SET( ch->act, PLR_WIZINVIS ) ) )
      {
         sprintf( buf, "&Y%s&w\r\n", ( ch->pcdata && ch->pcdata->bamfin && ch->pcdata->bamfin[0] != '\0' )
...
void do_mset( CHAR_DATA * ch, char *argument )
{
   ...
   if ( !str_cmp( arg2, "council2" ) )
   {
      ...
      if( !arg3 || arg3[0] == '\0' )
      {
         ...
         {
            --victim->pcdata->council2->members;
            if( victim->pcdata->council2->members < 0 )
               victim->pcdata->council2->members = 0;
            save_council( victim->pcdata->council2 );
         }
...
/*		if ( get_trust(ch) >= sysdata.level_modify_proto  */
            if( get_trust( ch ) >= LEVEL_GREATER
                ...
                || ( ch->pcdata->council2 && is_name( "aassign", ch->pcdata->council2->powers ) ) )
