...
bool is_wielding_poisoned( CHAR_DATA * ch )
{
   ...
   if( ( obj = get_eq_char( ch, WEAR_DUAL_WIELD ) ) != NULL && IS_OBJ_STAT( obj, ITEM_POISONED ) )
      return TRUE;
...
ch_ret multi_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_sixth_attack ) + ( dual_bonus * 3.5 ) ) / 5 );
   if( number_percent(  ) < chance )
   {
   ...
   chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_seventh_attack ) + ( dual_bonus * 4 ) ) / 6 );
   if( number_percent(  ) < chance )
   {
   ...
   chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_eighth_attack ) + ( dual_bonus * 4.5 ) ) / 7 );
   if( number_percent(  ) < chance )
   {
...
ch_ret one_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
    ...
    * Ntanel - Zebeid fliped class thac.
    */
   if( IS_NPC( ch ) )
   {
 ...
 */
   ...
   if( !IS_NPC( victim ) && victim->level >= LEVEL_IMMORTAL && victim->hit < 1 && !in_arena( victim ) )  /* Scion arenacode */
      victim->hit = 1;
   ...
   if( !npcvict && get_trust( victim ) >= LEVEL_IMMORTAL && get_trust( ch ) >= LEVEL_IMMORTAL && victim->hit < 1 && !in_arena( ch ) /* Scion arenacode */
       && !in_arena( victim ) )
      victim->hit = 1;
   update_pos( victim );
   ...
   if( victim->position == POS_DEAD )
   {
      ...
      if( !npcvict )
      {
         if( ch == victim )
            ...
            sprintf( log_buf, "%s has been killed by %s!", victim->name, ( IS_NPC( ch ) ? ch->short_descr : ch->name ) );
         ...
         if( victim->exp > exp_level( victim, victim->level ) )
            gain_exp( victim, ( exp_level( victim, victim->level ) - victim->exp ) / 4 );
...
*/
   if( ( ch->level - victim->level > 10 || victim->level - ch->level > 10 ) && ( !in_arena( ch ) || !in_arena( victim ) ) )
   {
      set_char_color( AT_IMMORT, ch );
...
*/
         if( victim->level == 0 )
         {
            level_ratio = URANGE( 1, ch->level, LEVEL_AVATAR );
         }
         else
         {
...
void new_dam_message( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt, OBJ_DATA * obj )
{
   ...
   else
   {
      ...
      sprintf( buf2, "Your %s %s $N%c &R[&Y%d&R]&W", attack, vp, punct, dam );
      sprintf( buf3, "$n's %s %s you%c &R[&Y%d&R]&W", attack, vp, punct, dam );
   }
...
void do_kill( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( victim ) )
   {
      if( !xIS_SET( victim->act, PLR_KILLER ) && !xIS_SET( victim->act, PLR_THIEF ) && !in_arena( victim ) )   /* Scion arenacode */
      {
...
void do_flee( CHAR_DATA * ch, char *argument )
{
   ...
   for( attempt = 0; attempt < 8; attempt++ )
   {
      ...
      if( !IS_NPC( ch ) )
      {
         ...
         los = ( exp_level( ch, ch->level + 1 ) - exp_level( ch, ch->level ) ) * 0.0015;
   ...
   los = ( exp_level( ch, ch->level + 1 ) - exp_level( ch, ch->level ) ) * 0.005;
   if( ch->level < LEVEL_AVATAR )
   {
