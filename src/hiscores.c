...
short get_position( char *keyword, char *name );
 ...
 * way. */
{
   ...
   for( entry = table->first_entry; entry; entry = entry->next )
   {
      pager_printf_color( ch, "&g| |     %s%2d. %-20.20s%15d     &g| |\r\n",
                          num <= 3 ? "&Y" : "&p", num, capitalize( entry->name ), entry->score );
      num++;
   }
 ...
 * if the player entered the table */
{
   ...
   if( pos != -1 && pos != old_pos && table != NULL )
   {
      ...
      for( vch = first_char; vch; vch = vch->next )
      {
         if( IS_NPC( vch ) )
            continue;
         sprintf( buf, "&R[&WINFO&R]&c %s has reached position %d in the table of '%s'", ch->name, pos, table->desc );
