...
void oedit_disp_prog_types( DESCRIPTOR_DATA * d )
{
   ...
   for( count = ACT_PROG; count < USE_PROG; count++ )
      ch_printf_color( d->character, "&g%2d&w) %s\r\n", count, mprog_type_to_name( count ) );
   send_to_char( "\r\nEnter type: ", d->character );
...
void oedit_disp_wear_menu( DESCRIPTOR_DATA * d )
{
   ...
   sprintf( buf, "\r\nWear flags: &c%s&w\r\nEnter wear flag, 0 to quit:  ", flag_string( obj->wear_flags, w_flags ) );
   send_to_char_color( buf, d->character );
...
void oedit_disp_menu( DESCRIPTOR_DATA * d )
{
    ...
    */
   sprintf( buf, "&g7&w) Wear flags  : &c%s\r\n" "&g8&w) Weight      : &c%d\r\n" "&g9&w) Cost        : &c%d\r\n" "&gA&w) Rent(unused): &c%d\r\n" "&gB&w) Timer       : &c%d\r\n" "&gC&w) Level       : &c%d\r\n"  /* -- Object level . */
            "&gD&w) Layers      : &c%d\r\n"
         ...
         switch ( UPPER( arg[0] ) )
         {
            ...
            case 'D':
               if( IS_SET( obj->wear_flags, ITEM_WEAR_BODY )
                   ...
                   || IS_SET( obj->wear_flags, ITEM_WEAR_LEGS ) || IS_SET( obj->wear_flags, ITEM_WEAR_WAIST ) )
               {
