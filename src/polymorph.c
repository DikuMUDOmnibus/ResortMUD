...
char *class_string( int bitvector )
{
   ...
   for( x = 0; x < MAX_CLASS; x++ )
      ...
      {
         strncat( buf, class_table[x]->who_name, MAX_STRING_LENGTH );
...
char *race_string( int bitvector )
{
   ...
   for( x = 0; x < MAX_RACE; x++ )
      if( IS_SET( bitvector, 1 << x ) )
      {
         strncat( buf, race_table[x]->race_name, MAX_STRING_LENGTH );
         strncat( buf, " ", MAX_STRING_LENGTH );
      }
   if( ( x = strlen( buf ) ) > 0 )
      buf[--x] = '\0';
...
void do_morphset( CHAR_DATA * ch, char *argument )
{
   ...
   if( ch->substate == SUB_REPEATCMD )
   {
      ...
      {
         strncpy( buf, morph->name, MAX_STRING_LENGTH );
         strncat( buf, " help", MAX_STRING_LENGTH );
         do_morphstat( ch, buf );
         return;
      }
   ...
   if( morph )
   {
      strncpy( arg1, morph->name, MAX_INPUT_LENGTH );
   ...
   else if( !str_cmp( arg2, "long" ) )
   {
      ...
      strncpy( buf, arg3, MAX_STRING_LENGTH );
      strncat( buf, "\r\n", MAX_STRING_LENGTH );
